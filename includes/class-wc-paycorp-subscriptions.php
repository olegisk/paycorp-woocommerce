 ...
 */
class WC_Paycorp_Subscriptions {
	const PAYMENT_ID = 'paycorp';
	...
	public function __construct() {
		...
		add_action(
			'woocommerce_subscription_payment_meta_input_' . self::PAYMENT_ID . '_paycorp_meta_token_id',
			...
			if ( count( $tokens ) > 0 ) {
				foreach ( $tokens as $token_id ) {
					...
					$subscription->add_order_note(
						sprintf(
						/* translators: 1: card name */                            __( 'Card: %s', 'paycorp' ),
		...
		$payment_meta[ self::PAYMENT_ID ] = array(
			'paycorp_meta' => array(
				'token_id' => array(
					...
					'label' => __( 'Card Token ID', 'paycorp' ),
				),
			),
		);
	...
	public static function payment_meta_input( $subscription, $field_id, $field_value, $meta_data ) {
		...
		echo '<option value="">' . __( 'Please select', 'paycorp' ) . '</option>';

		foreach ( $tokens as $token ) :
			/** @var WC_Payment_Token_Paycorp $token */

			$selected = (int) $field_value === $token->get_id() ? ' selected ' : '';
	...
	public static function validate_subscription_payment_meta( $payment_method_id, $payment_meta, $subscription ) {
		if ( self::PAYMENT_ID === $payment_method_id ) {
			if ( empty( $payment_meta['paycorp_meta']['token_id']['value'] ) ) {
				throw new Exception( 'A "Card Token ID" value is required.' );
			}

			$tokens = explode( ',', $payment_meta['paycorp_meta']['token_id']['value'] );
			foreach ( $tokens as $token_id ) {
				$token = new WC_Payment_Token_Paycorp( $token_id );
				if ( ! $token->get_id() ) {
					throw new Exception( 'This "Card Token ID" value is not found.' );
				}
				...
				if ( $token->get_gateway_id() !== self::PAYMENT_ID ) {
					throw new Exception( 'This "Card Token ID" value is invalid.' );
				}
			}
		}
	}
	...
	public static function save_subscription_payment_meta( $subscription, $meta_table, $meta_key, $meta_value ) {
		...
		if ( 'paycorp_meta' === $meta_table && 'token_id' === $meta_key ) {
			// Delete tokens
			delete_post_meta( $subscription->get_id(), '_payment_tokens' );
	...
	public static function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		try {
			...
			foreach ( $tokens as $token_id ) {
				...
				$gateway->charge_payment( $renewal_order, $token );
	...
	public static function maybe_render_subscription_payment_method( $payment_method_to_display, $subscription ) {
		...
		foreach ( $tokens as $token_id ) {
			...
			return sprintf(
			/* translators: 1: pan 2: month 3: year */                __( 'Via %1$s card ending in %2$s/%3$s', 'paycorp' ),
				$token->get_masked_card(),
	...
	public static function save_new_payment_method_option_html( $html, $gateway ) {
		...
		if ( self::wcs_cart_has_subscription() || self::wcs_is_payment_change() ) {
			...
			if ( false !== $status ) {
				...
				$html = $doc->saveHTML( $doc->documentElement ); // phpcs:ignore
			}
		}
	...
	private static function get_payment_gateway() {
		...
		throw new \Exception( 'The payment gateway is unavailable.' );
	}
